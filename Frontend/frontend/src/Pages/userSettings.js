...
const UserSettings = () => {
  ...
  const handleNameChange = async (e) => {
    ...
    if (!firstname.trim() && !lastname.trim()) {
      setMessage("Bitte geben Sie einen Vor- oder Nachnamen ein.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(data.message || "Fehler beim Ändern des Namens!");
        return;
      }

      setMessage_sucess(data.message || "Name erfolgreich geändert!");
      setFirstname("");
      setLastname("");
    } catch (error) {
      console.error("Fehler beim Ändern des Namens:", error);
      setMessage("Fehler beim Ändern des Namens!");
    }
  };
